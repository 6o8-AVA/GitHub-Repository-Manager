...
import type { HiddenClonedState } from '../main/storage';
...
function normalizeState(state: HiddenClonedState): HiddenClonedState {
...
function cloneState(state: HiddenClonedState): HiddenClonedState {
  return {
    orgs: [...state.orgs],
...
class HiddenClonedStore {
  private state: HiddenClonedState;
  ...
  private initializeFromStorage(): void {
    const stored = normalizeState(Storage.hiddenCloned.get());
    this.state = stored;
  ...
  private persist(): void {
    if (!this.isInitialized)
      return;
    if (Storage.isReady())
      void Storage.hiddenCloned.set(cloneState(this.state));
    this.emitter.fire();
  }
...
export const HiddenCloned = new HiddenClonedStore();
